...
public class ChessPiece {
	...
	public ChessPiece(Piece type, boolean whitePiece, Position position,
		...
		firstMoveWhite = new Move(0, -2, true);
		firstMoveBlack = new Move(0, 2,true);
		setupMoves();
		findValidPositions();
		playable = true;
		left = false;
	...
	public void findValidPositions() {
		...
		for (Move m : possibleMoves) {
			Position possiblePosition = new Position(
					position.getX() + m.getX(), position.getY() + m.getY());
	...
	private boolean noJumps(Position possiblePosition, Move possibleMove) {
		...
		try {
			if (squaresToCheck.isEmpty())
				return true; // If there are no squares in the way of the move,
								// there are no jumps.
		} catch (NullPointerException e) {
			return true;
		}
		for (Square s : squaresToCheck) {
			if (s.getPiece() != null) { // if there is a piece in the way of a
										// move
				// Do not allow move
				System.out.println("Disbaled Move(" + type + "from "
						+ this.position + " to " + possiblePosition);
				return false;
			}
		}
	...
	/**
	 * Get all the squares that are travelled upon during a move of a piece, not
	 * including the start and end squares.
	 *
	 * @param possibleMove
	 * @return
	 */
	private ArrayList<Square> getSquaresOnMove(Move possibleMove) {
		 ...
		 * up.
		 * 
		 * There are no squares 'squares on move' for pawns, because they can
		 * only move by one. The <b>bishop</b> can move diagonally by any amount
		 * as long as the diagonal route is unobstruced. The rook can move to
		 * any non-obstructed square with in it's row or column. The queen
		...
		case ROOK:
			...
			break;
		case BISHOP:// Diagonal, so movement in x and y direction will be equal
					// magnitude.
			if (x > 0 && y > 0) {// The move is going diagonally down to the
									// right.
				for (int i = 1; i < x; i++) {
					Position positionOnMove = new Position(x + i, y + i);
					if (positionOnMove.inBounds())
						squares.add(board.getSquare(positionOnMove));
				}
		...
		case KNIGHT:
			...
			break;
		case PAWN:// No squares between start and end positions of the piece, so
	...
	private boolean positionWithinBounds(Position possiblePosition) {
		return possiblePosition.getX() < 8 && possiblePosition.getX() >= 0
				&& possiblePosition.getY() < 8 && possiblePosition.getY() >= 0;
	}
	...
	public Move getMove(Position endPosition) {
		for (Move m : possibleMoves) {
			if (position.getX() + m.getX() == endPosition.getX()
					&& position.getY() + m.getY() == endPosition.getY()) {
				return m;
			}
		}
		return null;
